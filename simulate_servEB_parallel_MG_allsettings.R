...
set.seed(2025)
  ...
  alpha <- 0.05
  permreps <- 10000             # Permutationswiederholungen für kleine N
  ...
  simulate_data <- function(N, t_size, p_size, scenario, treat_resp, placeb_resp, mean_shift, sd_shift) {
    ...
    # Pain & Itch baseline
    # Pain & Itch baseline mit Korrelationsanpassung
    rho <- 0.4   # gewünschte Korrelation
    sd_pain <- 3
    sd_itch <- 3
    Sigma <- matrix(c(sd_pain^2, rho*sd_pain*sd_itch,
                      rho*sd_pain*sd_itch, sd_itch^2), nrow = 2)
  ...
  run_tests <- function(sim, alpha = 0.05, permreps = 10000) {
    ...
    # 4) rankFD::rankFD (statt rank.two.samples)
    out$mw_rankfd <- tryCatch({
      p1 <- rankFD::rankFD(wound_size_post ~ treatment, data = sim)$ANOVA.Type.Statistic[1, "p-Value"]
      p2 <- rankFD::rankFD(pain ~ treatment, data = sim)$ANOVA.Type.Statistic[1, "p-Value"]
      p3 <- rankFD::rankFD(itch ~ treatment, data = sim)$ANOVA.Type.Statistic[1, "p-Value"]
      padj <- p.adjust(c(p1, p2, p3), method = "holm")
      any(padj < alpha)
    }, error = function(e) NA)
  ...
  jobs <- jobs %>%
    filter(!(scenario != "power2" & shift != names(shift_settings)[1])) # behalte nur erste Shift bei type1/power1
  ...
  results_df <- do.call(rbind, lapply(seq_along(res_list), function(i) {
    ...
    data.frame(
      ...
      p_manovarm_mats = pvals$p_manovarm_mats,
  ...
  results_df <- results_df %>%
    mutate(
      ...
      manovarm_mats = ifelse(!is.na(p_manovarm_mats) & p_manovarm_mats < alpha, 1, 0),
  ...
  df_summary <- results_df %>%
    ...
    summarise(across(c(npmv, manovarm,manovarm_mats, manova, mw, mw_rankfd), mean), .groups = "drop")
  
  # Ergebnisse als TXT speichern
  write.table(df_summary, file = "simulation_results_servEB_all_11.2.txt",
