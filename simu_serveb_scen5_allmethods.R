...
# Datengenerator
# ===========================
simulate_data <- function(N, t_size, p_size, scenario, treat_resp, placeb_resp) {
  sim <- data.frame(
    patientID = 1:N,
...
df_summary <- results_df %>%
  group_by(sample_setting, scenario, effect) %>%
...
write.table(df_summary, paste0("df_summary_covariancehigher_typ1",format(Sys.Date(), "%d%m%Y"),".txt"))